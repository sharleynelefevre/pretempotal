        ...
        if os.path.isdir(path+foldername):
            for filename in os.listdir(path+foldername):
                ...
                for e in soup.find_all('EVENT'):
                    eventsTexts.append(e.text)
                
                timexsTexts = []
                for t in soup.find_all('TIMEX3'):
                    timexsTexts.append(t.text)
                
                signauxTexts = []
                for s in soup.find_all('SIGNAL'):
                    signauxTexts.append(s.text)
                    
                    
                for sent in tokenize.sent_tokenize(str(text)):
                    iterSent += 1
                    
                    # gestion de la ponctuation : eviter que les elements de ponctuation soient comptes comme des mots
                    transTable = str.maketrans(',()._`\'-', 8*' ')
                ...
                res.to_csv('dev/dataframe_id.csv', sep=';', encoding='utf-8') 
